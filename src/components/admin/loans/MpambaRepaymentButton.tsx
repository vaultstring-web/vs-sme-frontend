...
type Stage = 'idle' | 'form' | 'loading' | 'pending' | 'done' | 'error';

export default function MpambaRepaymentButton({
  loanId,
  borrowerName,
  borrowerPhone,
  remainingBalance,
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (parsed > remainingBalance) {
      setErrorMsg(`Amount cannot exceed the remaining balance of MWK ${remainingBalance.toLocaleString()}.`);
      return;
    }
    setStage('loading');
    setErrorMsg('');
    try {
      const res = await apiClient.post(`/mpamba/loans/${loanId}/repay`, { amount: parsed });
  ...
  async function checkStatus() {
    ...
    try {
      ...
      if (s === 'COMPLETED') {
        ...
        setStage('error');
        setErrorMsg('The customer did not complete the Mpamba payment.');
      }
    } catch {
    ...
    <div className="flex items-center justify-between bg-card px-4 py-4 sm:px-6 sm:py-5">
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="min-w-0">
          <h3 className="text-lg font-black tracking-tight text-foreground sm:text-xl">Request via Mpamba</h3>
          <p className="text-xs font-bold uppercase tracking-widest text-foreground/40">
            USSD push to {borrowerPhone}
          </p>
        </div>
      </div>
      <button
  ...
  return (
    <>
      <button
        type="button"
        onClick={open}
        className="flex w-full items-center justify-center gap-2 rounded-xl border border-primary-200 bg-primary-50 px-6 py-2.5 text-sm font-bold text-primary-700 shadow-sm transition-all hover:bg-primary-100 dark:border-primary-900/30 dark:bg-primary-900/10 dark:text-primary-400 sm:w-auto"
      >
        <Smartphone size={16} />
        Request via Mpamba
      </button>
        ...
        <AnimatePresence mode="wait">
            ...
            >
              <div className="rounded-2xl border border-border bg-slate-50/60 p-5 dark:bg-zinc-900/40 space-y-3">
                <div className="flex justify-between text-sm">
                  <span className="text-foreground/50 font-medium">Customer</span>
              ...
              <p className="text-xs text-foreground/50 text-center">
                The customer will receive a USSD prompt on their phone to authorise this payment.
              </p>
          ...
          {stage === 'pending' && (
            ...
            >
              ...
              <div>
                <h4 className="text-xl font-black text-foreground mb-1">USSD Sent</h4>
                <p className="text-sm text-foreground/60">
                  A USSD prompt has been sent to <strong>{borrowerPhone}</strong>. The payment will be
                  recorded automatically once the customer authorises it.
                </p>
              </div>
              {statusResult && (
          ...
          {stage === 'done' && (
            ...
            >
              ...
              <h4 className="text-2xl font-black text-foreground">Payment Received!</h4>
              <p className="text-foreground/60 text-sm">
                The Mpamba payment has been confirmed and the loan balance updated.
              </p>
              <button
                type="button"
                onClick={close}
